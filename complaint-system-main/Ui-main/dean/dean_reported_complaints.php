...
if ($userId <= 0 || strtolower((string)($_SESSION['role'] ?? '')) !== 'dean') {
    header('Location: login.php?error=' . urlencode('Please log in as dean.'));
    exit;
}
...
try {
    $studentStmt = $pdo->prepare(
        'SELECT first_name, last_name FROM student_profiles
         WHERE id = :student_id
           AND college_id = (SELECT college_id FROM dean_profiles WHERE user_id = :user_id LIMIT 1)
         LIMIT 1'
    );
    $studentStmt->execute([':student_id' => $studentId, ':user_id' => $userId]);
