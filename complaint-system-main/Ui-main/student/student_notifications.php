...
// student_topbar.php (included in <body> below) defines topbar_time_ago(),
// topbar_icon_for_type(), and topbar_link_for_type().
...
.notif-page-icon img { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; }
    ...
    <div class="notif-page-card">
        ...
        <?php else: ?>
            <?php
                ...
                function render_notif_page_item(PDO $pdo, array $notification, int $viewerStudentId = 0): void
                {
                    $notifType = (string)$notification['type'];
                    $isUnread = (int)$notification['is_read'] === 0;
                    $notifAvatar = $notifType === 'call_slip_issued'
                        ? topbar_call_slip_avatar($pdo, (string)($notification['ticket_type'] ?? 'complaint'), (int)($notification['ticket_id'] ?? 0))
                        : '';
            ...
            <?php if ($pageNewList): ?>
                <div class="notif-page-section notif-page-section-new">
                    ...
                    <div class="notif-page-list">
                        <?php foreach ($pageNewList as $notification): render_notif_page_item($pdo, $notification, $studentProfileId); endforeach; ?>
                    </div>
                </div>
            <?php endif; ?>
            <?php if ($pageEarlierList): ?>
                <div class="notif-page-section notif-page-section-earlier">
                    ...
                    <div class="notif-page-list">
                        <?php foreach ($pageEarlierList as $notification): render_notif_page_item($pdo, $notification, $studentProfileId); endforeach; ?>
                    </div>
                </div>
            <?php endif; ?>
        <?php endif; ?>
