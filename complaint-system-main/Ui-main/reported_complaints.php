...
if ($userId <= 0 || !in_array($role, ['admin', 'dean'], true)) {
...
try {
    $studentSql = 'SELECT sp.first_name, sp.last_name FROM student_profiles sp WHERE sp.id = :student_id';
    $studentParams = [':student_id' => $studentId];
    if ($role === 'dean') {
        $studentSql .= ' AND sp.college_id = (SELECT college_id FROM dean_profiles WHERE user_id = :user_id LIMIT 1)';
        $studentParams[':user_id'] = $userId;
    }
    $studentStmt = $pdo->prepare($studentSql . ' LIMIT 1');
    $studentStmt->execute($studentParams);
    $student = $studentStmt->fetch(PDO::FETCH_ASSOC);
    if (!$student) {
        throw new RuntimeException('Reported student was not found in your scope.');
    }
...
<title>Reported Complaints</title>
<link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600&display=swap" rel="stylesheet">
<link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
<style>
* { box-sizing: border-box; font-family: 'Poppins', sans-serif; }
body { margin: 0; background: #f4f6fb; color: #1f2937; }
.main { max-width: 1100px; margin: 0 auto; padding: 35px 20px; }
.header { display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 20px; }
h1 { margin: 0; font-size: 24px; }
.subtitle { color: #6b7280; margin: 5px 0 0; font-size: 13px; }
.back { color: #4f46e5; text-decoration: none; font-size: 13px; font-weight: 600; }
.complaint-count-badge { text-decoration: none; }
...
.ticket-no { color: #6b7280; font-size: 12px; font-weight: 500; }
        ...
        <?php foreach ($complaints as $complaint): ?>
            <section class="card">
                <div class="ticket-title">
                    ...
                    <span class="ticket-no"><?php echo e((string)$complaint['ticket_no']); ?></span>
                </div>
                <div class="detail-grid">
                    ...
                    <div class="detail full"><label>Person / Office Complained Of</label><p><?php echo e((string)$complaint['person_complained_of']); ?></p></div>
