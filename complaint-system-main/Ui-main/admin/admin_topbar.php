...
function topbar_link_for_type(string $type): string
{
    ...
    if ($type === 'student_status_update') {
        return 'admin_students.php';
    }
...
if (isset($_SESSION['user_id'])) {
    try {
        $profileStmt = $pdo->prepare(
            'SELECT u.username, u.profile_pic, ap.name
             FROM users u
        ...
        if ($profileRow) {
            $resolvedName = trim((string)($profileRow['name'] ?? ''));
            if ($resolvedName === '') {
                $resolvedName = (string)($profileRow['username'] ?? $topbarDisplayName);
            }
            $topbarDisplayName = $resolvedName;

            $storedPic = trim((string)($profileRow['profile_pic'] ?? ''));
            if ($storedPic !== '') {
                $topbarProfilePic = '../' . ltrim($storedPic, '/');
            }
        }
...
.notification-badge {
    ...
    padding: 2px 5px;
    border-radius: 50px;
    border: 2px solid #6d28d9; /* Matches topbar background */
}

.notification-dropdown {
    position: absolute;
    top: 45px;
    right: -10px; /* Adjusts alignment */
    width: 360px; /* Wider card */
    background: #fff;
    border-radius: 12px;
    ...
    overflow: hidden;
    z-index: 1000;
    cursor: default; /* Prevents text cursor inside */
...
.notification-item:hover {
    background: #f0f2f5; /* Facebook hover color */
}

/* Unread State */
.notification-item.unread {
    background: #f5f3f7; /* Light purple tint */
}
...
/* If you use an image instead of an icon */
.notif-icon img {
    width: 100%;
    height: 100%;
    border-radius: 50%;
...
.notif-content {
    ...
    padding-right: 15px; /* space for the unread dot */
}
...
@media (max-width: 768px) {
    ...
    .notification-dropdown {
        right: -50px; /* Adjust for mobile edges */
        width: 90vw;
        max-width: 360px;
    }
}
...
<div class="topbar">
    ...
    <a class="topbar-logo" href="admindashboard.php">
...
if (adminNotificationDropdown) {
    adminNotificationDropdown.addEventListener('click', function(e) {
        e.stopPropagation();
    });
}
...
window.onclick = function(event) {
    // If we aren't clicking inside the notification card, close it
    const notificationDropdown = document.getElementById('notificationDropdown');
    if (notificationDropdown && !event.target.closest('.notification-dropdown')) {
        notificationDropdown.style.display = 'none';
    }
    // If we aren't clicking inside the profile dropdown, close it
    const profileDropdown = document.getElementById('profileDropdown');
