...
function issue_call_slip(PDO $pdo, int $ticketId, string $ticketType, int $studentProfileId, string $issuedByRole, int $issuedByUserId, string $issuedByName = '', string $categoryName = '', string $reportDate = '', string $reportTime = '', string $officeMessage = '', string $reasonNote = ''): array
{
    try {
        ...
        // named in the complaint, e.g. "JOCELYN P. LUMACATUD issued a call
        ...
        // slip, e.g. "JOCELYN P. LUMACATUD issued a call slip for your
...
if ($flashMessage === '' || $flashType === 'success') {
    try {
        ...
        if (!$ticket) {
            ...
            // in the list as "New" while the SAS Director is already on it.
            ...
            // A complaint filed against a faculty/staff member has no reported
            // student, so the Call Slip is addressed to the faculty record.
            $isFacultyComplaint = (string)($ticket['reported_type'] ?? 'student') === 'faculty';
            $reportedFaculty = $isFacultyComplaint ? complaint_reported_faculty($pdo, $ticketId) : null;
            if ($reportedFaculty !== null) {
                $studentDisplayName = $reportedFaculty['name'];
                $studentDisplayEmail = $reportedFaculty['email'];
            }
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'issue_call_slip') {
                ...
                $result = $reportedFaculty !== null
                    ? issue_faculty_call_slip(
                        $pdo,
                        $ticketId,
                        $reportedFaculty['id'],
                        'admin',
...
.complaint-document { max-width: none !important; margin: 0 !important; padding: 0; overflow: hidden; }
.call-slip-history-card { max-width: none !important; margin: 0 !important; padding: 18px; min-height: 120px; }
.call-slip-history-title { color: #111827; font-size: 15px; font-weight: 700; margin-bottom: 12px; }
.call-slip-history-empty { color: #6b7280; font-size: 13px; line-height: 1.55; }
...
.call-slip-history-issuer { color: #111827; font-size: 13px; font-weight: 600; }
.call-slip-history-meta { color: #6b7280; font-size: 12px; line-height: 1.5; margin-top: 3px; }
.call-slip-history-status { display: inline-flex; align-items: center; padding: 4px 9px; border-radius: 999px; background: #dcfce7; color: #166534; font-size: 11px; font-weight: 700; }
...
.document-section-title .section-label { font-size: 16px; }
/* Label/value typography matched to admin_viewOnly_complaints.php */
...
@media (max-width: 900px) {
    ...
    .call-slip-history-card { min-height: 0; }
    .status-remarks-card, .response-timeline-card {
...
<div class="main">
    ...
    <div class="complaint-document card">
        <div class="document-heading">
            <div class="document-heading-left">
                <a href="admin_complaints.php" id="recordBackBtn" class="document-back-btn" aria-label="Back to Complaints" title="Back to Complaints">
    ...
    <div class="document-section">
        ...
        <?php if (!empty($ticket['attachments'])): ?>
            ...
            <div style="margin-top:12px;">
                <div class="label">Proof / Attachment</div>
                <?php if ($isImage): ?>
    ...
    <div class="call-slip-history-card card">
        <div class="call-slip-history-title">Call Slip History</div>
        <?php if (empty($callSlipHistory)): ?>
            <div class="call-slip-history-empty">No Call Slips have been sent for this complaint.</div>
        <?php else: ?>
            <div class="call-slip-history-list">
                <?php foreach ($callSlipHistory as $callSlip): ?>
                    <div class="call-slip-history-entry">
                        <div>
                            <div class="call-slip-history-issuer"><?php echo e((string)($callSlip['issuer_name'] ?? $callSlip['issued_by_role'] ?? 'Unknown issuer')); ?></div>
                            <div class="call-slip-history-meta">
                        ...
                        <span class="call-slip-history-status"><?php echo e(ucfirst((string)($callSlip['status'] ?? 'issued'))); ?></span>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
...
<form method="POST">
            ...
            <form method="POST" style="margin:0;">
                <div class="call-slip-paper">
                    ...
                    <div class="call-slip-body">
                        <div class="call-slip-form-title">CALL SLIP - SAS</div>
                        ...
                        <div class="call-slip-footer-row" style="justify-content:flex-end;">
                            <div class="call-slip-signature"><input type="text" name="issued_by" value="JOCELYN P. LUMACATUD"><span>SAS Director</span></div>
                        </div>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // admin came from (same filters/search/pagination/scroll position,
        // via normal browser back-navigation) instead of a fresh navigation
        // that would reset it back to page one.
        const backBtn = document.getElementById('recordBackBtn');
        if (backBtn) {
            backBtn.addEventListener('click', function (event) {
                const cameFromList = document.referrer && document.referrer.indexOf(window.location.origin) === 0;
                if (cameFromList && window.history.length > 1) {
                    event.preventDefault();
                    window.history.back();
                }
                // Otherwise let the plain href navigate to admin_complaints.php
